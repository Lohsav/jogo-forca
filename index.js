...
const palavras = ["banana", "brocolis", "camaro", "marrom", "picanha", "violeta", "cadeira", "leitura", "vinho", "camisa", "repolho", "caixa"];
...
// Convertendo a palavra em um array para poder manipular cada letra
...
function desenhaForca(qtdErros) {
     if (qtdErros === 1) {
        ...
        inicio.classList.remove("palitinho-inicio"); // Remova a classe que mantém a imagem visível
    }

    let todasAsImagens = document.getElementsByClassName("palitinhos");
    for (let i = 0; i < todasAsImagens.length; i++) {
        todasAsImagens[i].classList.remove("palitinho-visivel"); // Remova todas as classes que tornam as imagens visíveis
    }
